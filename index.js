/**
 * Util class to find config files.
 */
'use strict';
 ...
 * @private
 */
function getDirectoryEntriesSync(directory) {
  try {
...
function getDirectoryEntries(directory, callback) {
 ...
 */
FileFinder.prototype.findInDirectoryOrParentsSync = function(directory) {
  ...
  while (directory !== child) {
    ...
    if (getDirectoryEntriesSync(directory).indexOf(name) !== -1 && fs.statSync(path.resolve(directory, name)).isFile()) {
 ...
 */
FileFinder.prototype.findAllInDirectoryAndParentsSync = function(directory) {
  var cache = this.cache,
    child,
    dirs,
    name,
